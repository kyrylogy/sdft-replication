...
**"Trivial because it's part of the SDFT repo" — mostly, but with two surprises.** Reusing `DistilTrainer`/`DistilConfig` is the right call — the loss, masking, importance-sampling correction, and per-token KL are all already wired. What surprised us during verification: (1) the "frozen teacher" intuition is wrong under `main.py`'s defaults — with `sync_ref_model=True, alpha=0.01`, the teacher moves every step; calling it "frozen" only describes gradients, not weights. **This is the paper's recommended teacher choice per Appendix A.3** (ablated against frozen-base, which underperforms, and student-itself, which diverges). `main.py` is paper-faithful; the EMA isn't a TR-DPO-style add-on. (2) Only the vLLM path actually honors `generate_from_teacher=True`; both HF generate paths (`distil_trainer.py:1226, 1258`) always unwrap `self.model_wrapped` (the student). So if `use_vllm=False` (our case), `generate_from_teacher=True` would silently still sample from the student — a latent footgun. We set `generate_from_teacher=False` explicitly.
...
Comment in the file explains: under LoRA the student's base weights are frozen, so the EMA mix toward the teacher's base is a no-op even if the shape mismatch were patched. This is a **fidelity gap vs both `main.py` AND the paper's Appendix A.3 ablation** — we land on the "frozen base as teacher" arm, which A.3 explicitly identifies as the underperforming ablation. The deviation is forced by LoRA, not chosen. A proper paper-faithful LoRA-SDFT would need either (a) the teacher wrapped with its own LoRA + EMA over the adapter A/B matrices, or (b) merged-weight EMA each step. Neither is wired here — see "Open questions" below.
...
- **`sync_ref_model` is the paper's recommended teacher choice (Appendix A.3), and we lose it under LoRA.** A.3 ablates three teacher types: frozen base (underperforms — "fails to reflect improvements acquired during learning"), student-itself (diverges from on-policy feedback amplifying noise), and EMA-of-student (the recommended compromise). `main.py`'s `sync_ref_model=True, alpha=0.01` IS the EMA arm — it's paper-faithful, not a TR-DPO add-on. Our LoRA trainer disables it, landing on the underperforming "frozen base" arm by accident: the callback iterates `model.parameters()` and crashes on the LoRA A/B shape mismatch against the bare teacher. Even patched (filter named_parameters to base-only), EMA over the student's frozen base is a no-op — only the LoRA adapter actually moves under LoRA. To recover the paper's teacher behavior under LoRA you'd need either (a) wrap the teacher with its own LoRA and EMA over the adapter A/B matrices, or (b) materialize merged weights (`base + A@B`) each step and EMA-mix those into the teacher base. Neither is wired. Open research/implementation question for the cluster phase.
